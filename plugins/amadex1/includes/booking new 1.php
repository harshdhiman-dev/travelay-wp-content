        ...
        <script>
            (function() {
                ...
                document.getElementById('ahb-main').innerHTML =
                    ...
                    '<div class=\"ahb-field-group\" style=\"margin-bottom:12px;\">' +
                    '<label class=\"ahb-field-label\">Card Holder Name *</label>' +
                    '<input class=\"ahb-input\" type=\"text\" id=\"ahb-card-name\" placeholder=\"John Smith\" autocomplete=\"cc-name\">' +
                    '</div>' +
                    '<div class=\"ahb-field-group\" style=\"margin-bottom:12px;\">' +
                    '<label class=\"ahb-field-label\">Card Number *</label>' +
                    '<div id=\"ahb-collect-ccnumber\" style=\"min-height:46px;border:1px solid #ddd;border-radius:8px;background:#fff;\"></div>' +
                    '</div>' +
                    '<div class=\"ahb-payment-grid\" style=\"margin-bottom:12px;\">' +
                    '<div class=\"ahb-field-group\">' +
                    '<label class=\"ahb-field-label\">Expiry (MM/YY) *</label>' +
                    '<div id=\"ahb-collect-ccexp\" style=\"min-height:46px;border:1px solid #ddd;border-radius:8px;background:#fff;\"></div>' +
                    '</div>' +
                    '<div class=\"ahb-field-group\">' +
                    '<label class=\"ahb-field-label\">CVV *</label>' +
                    '<div id=\"ahb-collect-cvv\" style=\"min-height:46px;border:1px solid #ddd;border-radius:8px;background:#fff;\"></div>' +
                    '</div>' +
                    '</div>' +
                    '</div>' +
                    // PayPal panel
                    ...
                    '</div>' +
                    '<button id=\"ahb-collect-trigger\" type=\"button\" style=\"display:none;\"></button>' +
               ...
               // Load NMI Collect.js for card tokenization
                function ahbLoadCollectJs() {
                    if (!AHB_TOK_KEY || AHB_GATEWAY !== 'nmi') return;

                    function configureAhbCollect() {
                        if (typeof CollectJS === 'undefined') return;
                        try {
                            CollectJS.configure({
                                tokenizationKey: AHB_TOK_KEY,
                                paymentSelector: '#ahb-collect-trigger',
                                variant: 'inline',
                                styleSniffer: 'true',
                                currency: 'USD',
                                country: 'US',
                                fields: {
                                    ccnumber: { selector: '#ahb-collect-ccnumber', title: 'Card Number',     placeholder: '0000 0000 0000 0000' },
                                    ccexp:    { selector: '#ahb-collect-ccexp',    title: 'Expiration Date', placeholder: 'MM / YY' },
                                    cvv:      { selector: '#ahb-collect-cvv',      title: 'CVV',             placeholder: '***' }
                                },
                                customCss: {
                                    'color': '#333',
                                    'font-size': '14px',
                            ...
                            console.error('AHB CollectJS configure error:', e);
                        }
                    }
               ...
               window.ahbConfirm = function() {
                    ...
                    if (paymentMethod === 'credit_card' && AHB_GATEWAY === 'nmi' && !AHB_BYPASS) {
                        var btn = document.querySelector('.ahb-confirm-btn');
                        if (!_ahbCollectReady || typeof CollectJS === 'undefined') {
                            if (btn) { btn.disabled = false; btn.textContent = 'Confirm & Book'; }
                            alert('Payment fields are still loading. Please wait a moment and try again.');
                            return;
                        }
                        if (btn) { btn.disabled = true; btn.textContent = 'Securing Card…'; }
                        // Trigger tokenization via the hidden button that CollectJS is bound to
                        var trigger = document.getElementById('ahb-collect-trigger');
