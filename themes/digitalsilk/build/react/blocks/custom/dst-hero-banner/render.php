...
<div<?php echo $anchor_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
	...
	<span
		...
		aria-hidden="true"
		style="background-color: <?php echo esc_attr($overlay_color); ?>; opacity: <?php echo esc_attr($overlay_opacity); ?>;"></span>
	...
	<div class="c-hero-banner__inner" style="max-width: <?php echo esc_attr($content_width); ?>;">
		<div class="c-hero-banner__content">
			...
			<?php if (! empty($title)) : ?>
				<h1 class="c-hero-banner__title" style="color: <?php echo esc_attr($title_color); ?>;<?php echo $title_font_size ? ' font-size:' . esc_attr($title_font_size) . ';' : ''; ?> text-align: <?php echo esc_attr($title_align); ?>;">
		...
		<?php if (! empty($shortcode)) : ?>
			<div class="c-hero-banner__search">
				...
				// Extract any inline scripts so they execute properly outside the block wrapper.
				$shortcode_output = preg_replace_callback(
					'/<script([^>]*)>(.*?)<\/script>/is',
					function ($matches) {
						// Re-register extracted scripts to run after DOM is ready.
						add_action('wp_footer', function () use ($matches) {
							echo '<script' . $matches[1] . '>' . $matches[2] . '</script>'; // phpcs:ignore
						}, 99);
